...
export function inferPeriodFromQuestions(questions, fallback = '') {
  const list = questions || []
  for (const q of list) {
    const period = inferPeriod(q && (q.text || q.content))
    if (period === '大学') return '大学'
  }
  return STAGES.includes(fallback) ? fallback : ''
}
