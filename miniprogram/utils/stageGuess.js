// 从题文推断学段。ODE / 高等数学等必须标大学，不能落到默认「高中」。
...
function inferPeriodFromQuestions(questions, fallback) {
  const votes = { 小学: 0, 初中: 0, 高中: 0, 大学: 0 };
  (questions || []).forEach((q) => {
    const period = normalizePeriod(q && q.period) || inferPeriod((q && (q.text || q.content)) || '');
    if (period) votes[period] += 1;
  });
  if (votes['大学'] > 0) return '大学';
  const ranked = Object.keys(votes).sort((a, b) => votes[b] - votes[a]);
  if (votes[ranked[0]] > 0) return ranked[0];
