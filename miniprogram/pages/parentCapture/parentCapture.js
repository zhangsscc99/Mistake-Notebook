...
Page({
  ...
  goMine() {
    wx.reLaunch({ url: '/pages/parentMine/parentMine' });
  },
  ...
  loadRecentRecords() {
    ...
    callParent('childMistakes', { studentId, skip: 0 }).then((r) => {
      if (!r.success) return;
      const list = (r.data && r.data.questions) || [];
      this.setData({
        recentRecords: list.slice(0, 10).map((q) => ({
          id: q.id || q._id,
  ...
  viewRecord() {
    wx.reLaunch({ url: '/pages/parentMistakes/parentMistakes' });
  },
  ...
  _uploadOne(filePath, index) {
    const cloudPath = 'parent-help/' + Date.now() + '-' + index + '-' + Math.random().toString(36).slice(2, 8) + '.jpg';
    return new Promise((resolve, reject) => {
      wx.cloud.uploadFile({
        cloudPath,
        filePath,
