...
public class VisionReasoningService {
    ...
    private String buildQuestionSegmentationPrompt() {
        return """
                ...
                4. 注意事项：
                   ...
                   - 保持数学公式、符号的准确性
                   - 选择题要包含所有选项
                   - 题干可以是印刷体，也可以是手写体。手写的题目、公式、选项都要完整识别进 content。
                   - 如果卷面同时有「题目」和「学生作答/涂改」，content 只保留题目与选项，不要把订正笔迹或最终答案写进去。
                   - 整页都是手抄题时，按手写原文识别，不要只认印刷字。
                   - 手写潦草时按上下文和公式合理推断，不要编造整段没有的内容
    ...
    private String buildMultiPagePrompt(int pageCount) {
        return """
                ...
                3. 题干可以是印刷或手写。手写题目、公式、选项都要识别进 content。
                   若同时有学生作答或涂改，content 只保留题目本身。
                4. 每道题给出它出现的页码区间与位置。
